  ...
  return (
    <div style={{ border: "3px solid black" }}>
      <ul>
        ...
        <li>memo vs callBack:- useCallback same as useMemo but its return function instead callback return value</li>
      </ul>
      <input value={number} onChange={(e) => setNumber(e.target.value)} type="number" />
      <div style={styled}>{double}</div>
      <button onClick={() => setDark((pre) => (pre ? false : true))}>Change Theme</button>
    </div>
  )
})
